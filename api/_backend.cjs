...
async function getAdmin(req){const payload=verifyToken(parseCookies(req).dw_admin_token);if(!payload||payload.role!=='admin')return null;const row=await getPiUser(payload.uid);if(!row||row.is_admin!==true)return null;return{user:{id:row.pi_uid,pi_uid:row.pi_uid,username:row.username,is_admin:true},payload}}
