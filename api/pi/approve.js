...
module.exports = async function handler(req, res) {
  ...
  try {
    const { paymentId } = await getBody(req);

    if (!paymentId) {
      return sendJson(res, 400, { error: 'Missing paymentId' });
    }
    ...
    if (!response.ok) {
      return sendJson(res, response.status, { error: data || 'Pi approve request failed' });
    }

    return sendJson(res, 200, { approved: true, data });
  } catch (error) {
