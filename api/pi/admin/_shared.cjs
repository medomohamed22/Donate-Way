...
function makeAdminToken(user,maxAge=6*3600){return signPayload({uid:String(user.pi_uid),username:String(user.username||'pi-user'),exp:Date.now()+maxAge*1000})}
function cookie(token,maxAge=6*3600){return `dw_admin_token=${encodeURIComponent(token||'')}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${maxAge}`}
...
async function getAdmin(req){const payload=verifyToken(parseCookies(req).dw_admin_token);if(!payload)return null;const row=await getPiUser(payload.uid);if(!row||row.is_admin!==true)return null;return{user:{id:row.pi_uid,pi_uid:row.pi_uid,username:row.username,is_admin:true},payload}}
...
module.exports={env,headers,parseCookies,cookie,makeAdminToken,getAdmin,verifyPi,syncPiUser,buildQuery,fetchWithTimeout};
